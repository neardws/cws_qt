...
void MainWindow::initUI(){
    ...
    QString webstr=QString("file:///D:/offlineMap/offlinemap_demo/demo/1_0.html");//设置要打开的网页
...
//            if(interfaceList.at(j).humanReadableName().compare("以太网4")){
//                ui->NetInterface->addItem("Ethernet");
//            }else{
//                ui->NetInterface->addItem(interfaceList.at(j).humanReadableName());
//            }
        }
...
void MainWindow::onUdpStopButtonClicked(){
    if (udpStart){
        ...
        emit newLogInfo("UDP Thread Stoped");
    }
...
void MainWindow::on_pushButton_clicked(){
    if (threadStart){
        emit newLogInfo("Process thread is start");
    } else{
...
void MainWindow::on_pushButton_Stop_clicked(){
    if (threadStart){
        ...
        if(processThread != nullptr){
            ...
            disconnect(myudp, SIGNAL(newMessage(QString, QJsonObject)), processThread, SLOT(addMessage(QString, QJsonObject)));
       ...
       emit newLogInfo("Process thread is stoped.");
    }
...
void MainWindow::setRsu(const double &THRESHOLD, const double &DISTANCE_THRESHOLD){
    ...
    if(processThread == nullptr){
        ...
        connect(myudp, SIGNAL(newMessage(QString, QJsonObject)), processThread, SLOT(addMessage(QString, QJsonObject)));
...
void MainWindow::onUdpAppendMessage(const QString &from, const QJsonObject &message){
    ...
    if (type.compare(TYPE_MESSAGE) == 0){
        ...
        emit newLogInfo(message.find("id").value().toString()+"send a message."+ QString::number(m));
        ...
        emit newLogInfo("Message type error.");
    }
 ...
 * @return
 */
bool MainWindow::addMessageToDB(const QJsonObject &message, long long &receiverStamp){
    if (message.find("timeStamp").value().toString().toLongLong() != -666){
        int messageId = message.find("id").value().toString().toInt();
        long long timeStamp = message.find("timeStamp").value().toString().toLongLong();
        float speed = message.find("speed").value().toString().toFloat();
        float direction = message.find("direction").value().toString().toFloat();
        ...
        query.bindValue(":delay", delay);
        if(!query.exec()){
            emit newLogInfo("Insert Message failed."+ query.lastError().text());
            return false;
        } else {
            emit newLogInfo("Insert Message success");
            return true;
        }
    }
}
...
bool MainWindow::addResultToDB(const QJsonObject &result, bool sended){
    ...
    if(!query.exec()){
        emit newLogInfo("Insert Result failed."+ query.lastError().text());
        return false;
    } else {
        emit newLogInfo("Insert Result success");
        return true;
    }
}
...
bool MainWindow::addLogToDB(const QString &logInfo){
    ...
    if(!query.exec()){
        emit newLogInfo("Insert Log failed."+ query.lastError().text());
        return false;
    } else {
        emit newLogInfo("Insert Log success");
        return true;
    }
}
